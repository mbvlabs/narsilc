...
func main() {
	...
	log.SetPrefix("[narsilc-test-setup] ")

	if len(os.Args) < 2 {
		fmt.Fprintln(os.Stderr, "usage: narsilc-test-setup <install|start>")
		os.Exit(1)
	}
	...
	default:
		fmt.Fprintf(os.Stderr, "unknown command: %s\nusage: narsilc-test-setup <install|start>\n", os.Args[1])
		os.Exit(1)
	}
}
...
func startPostgreSQL() error {
	...
	if _, err := os.Stat(filepath.Join(dataDir, "PG_VERSION")); os.IsNotExist(err) {
		...
		if err := appendToFile(confPath,
			"\n# narsilc-test-setup configuration\n"+
...
func configurePgHBA(hbaPath string) error {
	content := `# pg_hba.conf - generated by narsilc-test-setup
