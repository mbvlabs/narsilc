...
//go:embed andurel_scan.go
var andurelScanSrc []byte
...
func generate(req *plugin.GenerateRequest, options *opts.Options, enums []Enum, structs []Struct, queries []Query) (*plugin.GenerateResponse, error) {
	...
	if options.AndurelRowMapping() {
		code, err := renderAndurelScan(options.Package, req.SqlcVersion, options.OmitSqlcVersion)
		if err != nil {
			return nil, err
		}
		output["andurel_scan.go"] = string(code)
	}
...
func renderAndurelScan(packageName, sqlcVersion string, omitVersion bool) ([]byte, error) {
	var header strings.Builder
	header.WriteString("// Code generated by sqlc. DO NOT EDIT.\n")
	if !omitVersion {
		header.WriteString("// versions:\n")
		header.WriteString("//   sqlc ")
		header.WriteString(sqlcVersion)
		header.WriteString("\n")
	}
	header.WriteString("\n")

	body := strings.Replace(string(andurelScanSrc), "package golang\n", "package "+packageName+"\n", 1)
	return format.Source([]byte(header.String() + body))
}
